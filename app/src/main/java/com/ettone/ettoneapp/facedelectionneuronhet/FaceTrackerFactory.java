...
class FaceTrackerFactory implements MultiProcessor.Factory<Face> {
    ...
    private FaceDetectedListener _faceDetectedListener;

    FaceTrackerFactory(GraphicOverlay graphicOverlay,FaceDetectedListener faceDetectedListener) {
        mGraphicOverlay = graphicOverlay;
        this._faceDetectedListener = faceDetectedListener;
    }

    @Override
    public Tracker<Face> create(Face face) {
        FaceGraphic graphic = new FaceGraphic(mGraphicOverlay,_faceDetectedListener);
...
class FaceGraphic extends TrackedGraphic<Face> {
    ...
    FaceGraphic(GraphicOverlay overlay,FaceDetectedListener faceDetectedListener) {
        super(overlay);

        mCurrentColorIndex = (mCurrentColorIndex + 1) % COLOR_CHOICES.length;
        final int selectedColor = COLOR_CHOICES[mCurrentColorIndex];
        _faceDetectedListener= faceDetectedListener;
    ...
    void updateItem(Face face) {
        mFace = face;
        _faceDetectedListener.isFaceDetected( true );
        postInvalidate();
    }
